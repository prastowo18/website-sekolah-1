...
type FacilityStatusFilter = "all" | "active" | "inactive";

function firstValue(value: string | string[] | undefined): string {
  return Array.isArray(value) ? (value[0] ?? "") : (value ?? "");
}

function normalizeStatus(value: string): FacilityStatusFilter {
...
function buildHref({
  ...
  return query
    ? `/konsol-8m4q7x2k9v6d/fasilitas?${query}`
    : "/konsol-8m4q7x2k9v6d/fasilitas";
}
...
export default async function FacilityPage({
  ...
  const totalPages = Math.max(1, Math.ceil(totalFacilities / PAGE_SIZE));
  ...
  const firstItem =
    totalFacilities === 0 ? 0 : (currentPage - 1) * PAGE_SIZE + 1;

  const lastItem = Math.min(currentPage * PAGE_SIZE, totalFacilities);
  ...
  return (
    <div className="space-y-6">
      <div className="flex flex-col gap-4 sm:flex-row sm:items-center sm:justify-between">
        <div>
          ...
          <p className="mt-1 text-sm text-muted-foreground">
            Kelola fasilitas yang tersedia di lingkungan sekolah.
          </p>
        </div>

        {canEdit ? <FacilityFormDialog /> : null}
      </div>
          ...
          <form className="grid gap-3 md:grid-cols-[1fr_220px_auto_auto]">
            ...
            {q || status !== "all" ? (
              <Button variant="outline" asChild>
                <Link href="/konsol-8m4q7x2k9v6d/fasilitas">
      ...
      <FacilityTable facilities={serializedFacilities} canEdit={canEdit} />

      <div className="flex flex-col gap-3 text-sm text-muted-foreground sm:flex-row sm:items-center sm:justify-between">
        <p>
          Menampilkan {firstItem}–{lastItem} dari {totalFacilities} fasilitas.
        </p>
