...
export const metadata: Metadata = {
  title: "Ekstrakurikuler | Panel Administrasi",
...
function buildHref({
  ...
  return query
    ? `/konsol-8m4q7x2k9v6d/ekstrakurikuler?${query}`
    : "/konsol-8m4q7x2k9v6d/ekstrakurikuler";
}
...
export default async function ExtracurricularPage({
  ...
  const serialized = extracurriculars.map((item) => ({
    ...item,
  ...
  return (
    <div className="space-y-6">
      <div className="flex flex-col gap-4 sm:flex-row sm:items-center sm:justify-between">
        <div>
          ...
          <p className="mt-1 text-sm text-muted-foreground">
            Kelola kegiatan, jadwal, pembina, dan kelompok kelas.
          </p>
        </div>

        {canEdit ? <ExtracurricularFormDialog /> : null}
      </div>
          ...
          <form className="grid gap-3 md:grid-cols-[1fr_220px_auto_auto]">
            ...
            {q || status !== "all" ? (
              <Button variant="outline" asChild>
                <Link href="/konsol-8m4q7x2k9v6d/ekstrakurikuler">
      ...
      <ExtracurricularTable extracurriculars={serialized} canEdit={canEdit} />

      <div className="flex flex-col gap-3 text-sm text-muted-foreground sm:flex-row sm:items-center sm:justify-between">
        <p>
          Menampilkan {firstItem}–{lastItem} dari {total} ekstrakurikuler.
        </p>
