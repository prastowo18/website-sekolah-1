...
import { postCategoryFormSchema, postCategoryIdSchema } from "./schemas";
import type { PostCategoryActionState, PostCategoryFieldName } from "./types";
...
function validationErrorState(error: z.ZodError): PostCategoryActionState {
  const errors = z.flattenError(error).fieldErrors;

  return {
    status: "error",
    message: "Periksa kembali data kategori berita.",
    fieldErrors: errors as Partial<Record<PostCategoryFieldName, string[]>>,
  };
}
...
function invalidSlugState(): PostCategoryActionState {
  return {
    status: "error",
    message: "Slug kategori berita tidak valid.",
    fieldErrors: {
...
export async function createPostCategoryAction(
  ...
  const parsed = postCategoryFormSchema.safeParse(getFormValues(formData));
  ...
  try {
    const createdCategory = await prisma.$transaction(async (transaction) => {
      const category = await transaction.postCategory.create({
        data: {
    ...
    console.error("Gagal menambahkan kategori berita.", error);
    ...
    return {
      status: "error",
      message: "Kategori berita gagal ditambahkan. Silakan coba kembali.",
    };
  }
}
...
export async function updatePostCategoryAction(
  ...
  const idParsed = postCategoryIdSchema.safeParse({
    id: formData.get("id"),
  });
  ...
  if (!idParsed.success) {
    return {
      status: "error",
      message: "ID kategori berita tidak valid.",
    };
  }
  ...
  try {
    const currentCategory = await prisma.postCategory.findUnique({
      where: {
        id: idParsed.data.id,
      },
      select: postCategorySelect,
    ...
    await prisma.$transaction(async (transaction) => {
      const updatedCategory = await transaction.postCategory.update({
        where: {
    ...
    console.error("Gagal memperbarui kategori berita.", error);
    ...
    return {
      status: "error",
      message: "Kategori berita gagal diperbarui. Silakan coba kembali.",
    };
  }
}
...
export async function deletePostCategoryAction(
  ...
  const parsed = postCategoryIdSchema.safeParse({
    id: formData.get("id"),
  });
  ...
  try {
      ...
      await transaction.auditLog.create({
        data: {
          ...
          oldValue: {
            ...
            affectedPostCount: currentCategory._count.posts,
          },
        },
      });
    });
    ...
    const affectedPostCount = currentCategory._count.posts;
    ...
    console.error("Gagal menghapus kategori berita.", error);

    return {
      status: "error",
      message: "Kategori berita gagal dihapus. Silakan coba kembali.",
    };
  }
}
