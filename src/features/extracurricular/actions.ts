...
function validationErrorState(
  ...
  return {
    status: "error",
    message:
      "Periksa kembali data ekstrakurikuler.",
    fieldErrors:
      errors as Partial<
        Record<
          ExtracurricularFieldName,
...
function invalidSlugState(): ExtracurricularActionState {
  return {
    status: "error",
    message:
      "Slug ekstrakurikuler tidak valid.",
    fieldErrors: {
      slug: [
...
function uniqueSlugState(): ExtracurricularActionState {
  return {
    status: "error",
    message:
      "Slug sudah digunakan oleh ekstrakurikuler lain.",
    fieldErrors: {
...
export async function createExtracurricularAction(
  ...
  const parsed =
    extracurricularFormSchema.safeParse(
  ...
  try {
    const createdExtracurricular =
      await prisma.$transaction(
        async (transaction) => {
          const extracurricular =
            await transaction.extracurricular.create({
              data: {
    ...
    return {
      status: "error",
      message:
        "Ekstrakurikuler gagal ditambahkan. Silakan coba kembali.",
    };
  }
}
...
export async function updateExtracurricularAction(
  ...
  const idParsed =
    extracurricularIdSchema.safeParse({
      id: formData.get("id"),
    });
  ...
  if (!idParsed.success) {
    return {
      status: "error",
      message:
        "ID ekstrakurikuler tidak valid.",
    };
  }
  ...
  try {
    ...
    await prisma.$transaction(
      async (transaction) => {
        const updatedExtracurricular =
          await transaction.extracurricular.update({
            where: {
        ...
        await transaction.auditLog.create({
          data: {
            actorId: session.user.id,
            action:
              "EXTRACURRICULAR_UPDATED",
            entity: "Extracurricular",
            entityId:
              currentExtracurricular.id,
            oldValue:
              currentExtracurricular,
            newValue:
              updatedExtracurricular,
          },
        });
      },
    );
    ...
    return {
      status: "error",
      message:
        "Ekstrakurikuler gagal diperbarui. Silakan coba kembali.",
    };
  }
}
...
export async function deleteExtracurricularAction(
  ...
  try {
    ...
    return {
      status: "error",
      message:
        "Ekstrakurikuler gagal dihapus. Silakan coba kembali.",
    };
  }
}
